		...
		tabs: [
			{
				id: "overview",
				label: "Overview",
				body: `Automated follow spotlights are a solved problem — if you have tens of thousands of dollars to spend on a BlackTrax system, Robe RoboSpot, or any of the other proprietary industry leaders. My senior thesis tackled the same problem with a Raspberry Pi, a $125 DMX interface, and some infrared LEDs soldered to a battery pack.
...
Two parallel C processes handle the actuation stack. A fixed-rate DMX Daemon manages all protocol timing and FTDI (USB) interface I/O via the ENTTEC DMX USB Pro, selectively overwriting pan/tilt channels from shared memory when tracking is active. A separate Main Control process handles coordinate transformation and state machine logic without competing for the daemon's clock cycle. Commands are issued from the lightboard using a range-based action table — holding a DMX value in a defined range for one second triggers the corresponding function — the same pattern used by professional fixtures like the Vari-Lite VL2500.`,
			},
			{
				id: "hardware-design",
				label: "Hardware Design",
				body: `The IR beacon array was designed from scratch around the tracking system's optical requirements. Three 260° 850 nm emitters were wired in series with a 30Ω current-limiting resistor to a 6V AA battery pack, sized to sit in a performer's back pocket with leads running up to shoulder and forehead mounting points — the same routing used for wireless vocalist mic elements, so it integrates naturally into existing costume workflows.
